  ...
  <div class="table_foot">
    ...
    <p class="f_l">
      ...
      <input type="button" onclick="content.forward();" value="转发到微博" class="button_style_1" title="将选择的文章转发到微博"/>
      <?php endif;?>
      ...
      <?php endif;?>
      <input type="button" onclick="content.allPushMobile();" value="推至移动端" class="button_style_1" title="批量推至移动端"/>
      <input type="button" onclick="content.changeWeight();" value="修改权重" class="button_style_1" title="批量修改权重"/>
      <input type="button" onclick="content.pushToPlace();" value="推送到专题" class="button_style_1" title="批量推送到专题"/>      
    </p>
  </div>
...
var row_template = '<tr id="row_{contentid}" right_menu_id="right_menu_{modelalias}" modelid="{modelid}" model="{modelalias}" data-url="{url}">\
	                	...
	                	<td><a title="查看内容" href="javascript:content.view({contentid});"><div class="icon {modelalias}"></div></a><?php if(!empty($childids)):?><a href="javascript: content.category({catid});" class="c_blue">[{catname}]</a> <?php endif?><a href="javascript:;" onclick="content.edit({contentid});" style="color:{color}" tips="ID：{contentid}<br />点击：{pv}次<br />评论：{comments}条<br />来源：{source_name}<br />Tags：{tags}<br />创建：{createdbyname}（{created}）<br />修改：{modifiedbyname}（{modified}）<br />审核：{checkedbyname}（{checked}）" class="title_list">{title}</a> {section}{thumb_icon} {note} {lock}{score} {comment_flag} {tweeted}</td>\
	                 	<td class="t_c">'+menuIco+'<a href="{url}" target="_blank"><img src="images/view.gif" alt="访问" title="访问" width="16" height="16" /></a> &nbsp;<img src="images/edit.gif" alt="编辑" title="编辑" width="16" height="16" class="hand edit"/> &nbsp;<img src="images/delete.gif" alt="删除" title="删除" width="16" height="16" class="hand delete" /></td>\
	                	<td class="t_r">{pv}</td>\
