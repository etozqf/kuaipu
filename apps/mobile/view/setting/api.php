    ...
    <table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_form mar_l_8">
        ...
        <tr>
            ...
            <td>
                ...
                <label class="islogin" type="<?=$share['alias']?>" ><input type="radio" name="config[api][<?=$share['alias']?>][open]" value="0"<?=form_element::checked(!$setting['api'][$share['alias']]['open'])?> <?php if($setting['api'][$share['alias']]['open'] && ($setting['login']['mobile'][$share['alias']] || $setting['login']['pad'][$share['alias']])): ?> disabled="disabled"<?php endif;?> /> 关闭</label>
            </td>
        </tr>
        <tr>
            <th>App ID：</th>
    ...
    $(function() {
        ...
        $('.islogin').click(function(){
            ...
            if ($(this).find('input').attr('disabled')) {
                ct.error(typename+'登陆配置已开启，如关闭将无法进行第三方登陆！');
                return false;
            }
        });
    });
