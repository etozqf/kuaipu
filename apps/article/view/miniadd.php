	...
	<form name="article_add" id="article_add" method="POST" action="?app=article&controller=article&action=add">
		<div class="mini-main">
			...
			<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_form">
                ...
                <tr>
                    <th>评论：</th>
                    <td colspan="3"><label><input type="checkbox" name="allowcomment" id="allowcomment"  class="checkbox_style"/>允许</label></td>
                </tr>
                <tr>
                    <th>状态：</th>
                    <td colspan="3">
...
$('#article_add').ajaxForm(function(json) {
	if (json.state){
		...
		if (json.url) {
			...
			// 非发布状态
			success.find('[data-role="message"]').html('转载成功，进入待发');
		...
		success.find('.success-weibo').find('td').html('<a href="<?php echo ADMIN_URL;?>?app=weibo&controller=weibo&action=index&id='+json.contentid+'" class="button_style_1" style="padding: 1px 6px 4px;" target="_blank">转发</a>');
    ...
    function categoryReady() {
        catid.data('selectree') && catid.data('selectree').bind('checked', function(item) {
            ...
            item.catid && content.isModelEnabled(item.catid);
       		$.ajax({
					url:'?app=system&controller=toolbox&action=isAllowcomment&catid='+item.catid,
					type:'get',
					dataType:'json',
					success:function(data){
							if(data==0){
								$('#allowcomment').attr("checked",false);
								$('#allowcomment').val('0');
							}else{
								$('#allowcomment').attr("checked",true);
								$('#allowcomment').val('1');
							}
						}
           		});
...
$('#allowcomment').bind('click',function(){
	this.value=1;
});
