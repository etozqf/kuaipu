...
var contribution = {
	...
	reject : function(contributionid) {
        if (contribution.islock(contributionid)) {
            ct.error('当前稿件已被锁定，无法修改！');
            return;
        }
		if (contributionid === undefined) {
			contributionid = tableApp.checkedIds();
    ...
    islock: function (contributionid)
    {
        // error
        var response = $.ajax({url: '?app=contribution&controller=index&action=islock&contributionid='+contributionid, async: false, dataType: "json"}).responseText;
        var a = new Function("return "+response);
        response = a();
        if (!response.state)
        {
            $("#row_"+contributionid+" img[src='images/lock.gif']").remove();
        }
