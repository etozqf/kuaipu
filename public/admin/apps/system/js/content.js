...
var content = 
{
    ...
    //批量推送到移动端
    allPushMobile: function(contentid){
        if (contentid == undefined){
            contentid = tableApp.checkedIds();
            if(!contentid.length){
                ct.warn('请选择要操作的记录');
        ...
        ct.form('添加关键词链接','?app=mobile&controller=content&action=getcategory&contentid='+contentid, 400, 300, function(json){
            if(json.state){
                ct.tips('操作成功！', 'ok');
            }else{
                ct.tips('部分'+json.error, 'error');
	...
	//推送到专题	
	pushToPlace:function(contentid){
		if (contentid == undefined){
            contentid = tableApp.checkedIds();
            if(!contentid.length){
                ct.warn('请选择要操作的记录');
	...
	//批量修改权重
	changeWeight:function (contentid)
	{
		if (contentid == undefined){
            contentid = tableApp.checkedIds();
            if(!contentid.length){
		...
		var dialog = ct.iframe({
                url:'?app=system&controller=content&action=weight',
                width:800,
                height:120
            }, {
                picked:function(weight){
					$.getJSON('?app=system&controller=content&action=weight', {
                        contentid:contentid,
                        weight:weight
                    }, function(json) {
                        if (json && json.state) {
