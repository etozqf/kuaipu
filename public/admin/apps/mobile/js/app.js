(function() {
    ...
    function loadItems(type, version) {
        ...
        $.getJSON('?app=mobile&controller=setting&action=app_list&type='+type+'&version='+version+'&model='+stylemodel, function(json) {
            listMenu.empty();
        ...
        $item.click(function() {
            ...
            $item.dropdialog({
                content: appid
                    ...
                    : ('?app=mobile&controller=setting&action=app_add' + '&type=' + type + '&version=' + version + '&model=' + stylemodel),
                width: 500,
                region: region,
                closeOnBlur: false,
                closeButton: false,
    ...
    function renderTag() {
        tagList.children('li').bind('click', function() {
            ...
            stylemodel = li.attr('data-model') || 'drawer';
            $('.'+stylemodel).show().siblings('.app-row-title').hide();
            loadItems(type, version);
        });
    }
    ...
    window.MobileApp = {
        ...
        update:function(id, data, callback, error) {
            $.post('?app=mobile&controller=setting&action=app_update&appid=' + id + '&type=' + type + '&version=' + version + '&model=' + stylemodel, data, function(json) {
                if (json && json.state) {
