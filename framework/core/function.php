...
{
	//当同一个脚本多次调用table函数，直接返回，不用再去连接redis读取缓存
    static $_staticCache;
...
function pages($total, $page = 1, $pagesize = 20, $offset = 2, $url = null, $mode = false, $tag='li', $class="row",  $prev="上一页", $next="下一页")
{
	...
	$str .= $page == 1 ? $p_start_tag.'<a href="javascript:void(0);" class="'.$class.'">1</a>'.$p_end_tag : $p_start_tag.'<a href="'.pages_url($url, 1, $mode).'">1'.($from > 2 && $more ? ' ...' : '').'</a>'.$p_end_tag;
	if ($to >= $from)
	{
		for($i = $from; $i <= $to; $i++)
		{
			$str .= $i == $page ? $p_start_tag.'<a href="javascript:void(0);" class="'.$class.'">'.$i.'</a>'.$p_end_tag : $p_start_tag.'<a href="'.pages_url($url, $i, $mode).'">'.$i.'</a>'.$p_end_tag;
		}
	}
